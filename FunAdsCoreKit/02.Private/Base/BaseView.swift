...
open class BaseView: UIView, BackgroundViewable, MarginAdjustable {
    ...
    open func installContentViewReward(_ contentView: UIView, insets: UIEdgeInsets = UIEdgeInsets.zero) {
        contentView.translatesAutoresizingMaskIntoConstraints = false
        backgroundView.addSubview(contentView)
        contentView.topAnchor.constraint(equalTo: backgroundView.topAnchor, constant: 0).isActive = true
        contentView.bottomAnchor.constraint(equalTo: backgroundView.bottomAnchor, constant: 0).isActive = true
        ...
        contentView.heightAnchor.constraint(equalToConstant: UIScreen.main.bounds.height).with(priority: UILayoutPriority(rawValue: 200)).isActive = true
    }
    ...
    open func installBackgroundView(_ backgroundView: UIView, insets: UIEdgeInsets = UIEdgeInsets.zero) {
        ...
        regularWidthLayoutConstraints = [
            ...
            backgroundView.widthAnchor.constraint(equalToConstant: UIDevice.containsheight).with(priority: UILayoutPriority(rawValue: 950)),
        ]
    }
    ...
    /**
     ...
     - Parameter insets: The amount to inset the content view from the horizontal margins and vertical edges.
       Default is zero inset.
     */
     func installBackgroundVerticalView(_ backgroundView: UIView, insets: UIEdgeInsets = UIEdgeInsets.zero) {
        backgroundView.translatesAutoresizingMaskIntoConstraints = false
        if backgroundView != self {
            backgroundView.removeFromSuperview()
        }
        ...
        regularWidthLayoutConstraints = [
            backgroundView.leftAnchor.constraint(greaterThanOrEqualTo: layoutMarginsGuide.leftAnchor, constant: insets.left).with(priority: UILayoutPriority(rawValue: 900)),
            backgroundView.rightAnchor.constraint(lessThanOrEqualTo: layoutMarginsGuide.rightAnchor, constant: -insets.right).with(priority: UILayoutPriority(rawValue: 900)),
            backgroundView.widthAnchor.constraint(lessThanOrEqualToConstant: 500).with(priority: UILayoutPriority(rawValue: 1000)),
            backgroundView.widthAnchor.constraint(equalToConstant: UIDevice.containsheight).with(priority: UILayoutPriority(rawValue: 1000)),
        ]
    }

    func installBackgroundVerticalViewImage(_ backgroundView: UIView, insets: UIEdgeInsets = UIEdgeInsets.zero) {
       backgroundView.translatesAutoresizingMaskIntoConstraints = false
       if backgroundView != self {
           backgroundView.removeFromSuperview()
       }
       ...
       regularWidthLayoutConstraints = [
           backgroundView.leftAnchor.constraint(greaterThanOrEqualTo: layoutMarginsGuide.leftAnchor, constant: insets.left).with(priority: UILayoutPriority(rawValue: 900)),
           backgroundView.rightAnchor.constraint(lessThanOrEqualTo: layoutMarginsGuide.rightAnchor, constant: -insets.right).with(priority: UILayoutPriority(rawValue: 900)),
           backgroundView.widthAnchor.constraint(lessThanOrEqualToConstant: 700).with(priority: UILayoutPriority(rawValue: 1000)),
           backgroundView.widthAnchor.constraint(equalToConstant: UIDevice.containsheight).with(priority: UILayoutPriority(rawValue: 950)),
       ]
   }
    /*
